...
struct isRegister<cutlass::AlignedArray<T, N>> : cuda::std::true_type {};

template<class T, unsigned int N>
struct isRegister<cutlass::Array<T, N>> : cuda::std::true_type {};
...
struct VAA {
    ...
    requires cutlass::detail::is_Array_v<Registers> &&
        cuda::std::is_same_v<typename Registers::value_type, Element>
    __device__ __forceinline__
    void operator()(Element* __restrict__ const& gS, const Registers& registers) const {
        #pragma unroll
...
struct VAA<Arch, cute::half_t> {
    ...
    __device__ __forceinline__
    void operator()(cute::half_t* __restrict__ const& gS, const Registers& registers) const {
...
struct VAA<Arch, cute::bfloat16_t> {
    ...
    __device__ __forceinline__
    void operator()(cute::bfloat16_t* __restrict__ const& gS, const Registers& registers) const {
...
struct VAA<900, float> {
    ...
    __device__ __forceinline__
    void operator()(float* __restrict__ const& gS, const Registers& registers) const {
...
struct Combine {
    ...
    >
    requires(TensorValueType<Element> && cute::is_tensor_v<Activations>)
    __device__ __forceinline__
    void operator()(Element* __restrict__ workspace,
        ...
        /*using BlockTiler = cute::Shape<cute::Int<BLOCK_M>,
                                    cute::Int<BLOCK_N>,
                                    cute::Int<BLOCK_K_FULL>>;
        ...
        const auto tilesK = K / cute::get<1>(tiler);
...
__global__ __maxnreg__(128) void deviceCombine(cuda::std::byte* __restrict__ p) {
    ...
    const auto* __restrict__ inputs = CAST_TO(Element, p + sizeof(unsigned int) * M);
    const auto activations = make_tensor(
        cute::make_gmem_ptr(CAST_TO(Element, p + sizeof(unsigned int) * M + sizeof(Element) * M * K)),
    ...
    combineOp(scratch, tokenIndices, regs, inputs, activations, M, K, blockIdx.x, M);
}
...
void hostCombine() {
    ...
    // zerofill activations
    CHECK_ERROR_EXIT(cudaMemsetAsync(a + sizeof(unsigned int) * M + aSize, 0, aSize, playStream));
    deviceCombine<M, K, inputValueType><<<1, 1, SHARED_SIZE, playStream>>>(a);
